...
const birdFS = `
    ...
    void main() {
        // Emerald/Mint green color inspired by the site theme
        vec3 themeColor = vec3(0.06, 0.72, 0.5); // equivalent to #10b981
        float z2 = 0.2 + ( 1000. - z ) / 1000. * vColor.x;
        gl_FragColor = vec4( themeColor * z2, 0.4 ); // Semi-transparent green birds
    }
`;
...
export const initGPGPUBirds = () => {
    ...
    // Instead of white background, keep it completely transparent (alpha: true)
    
    scene.fog = new THREE.Fog( 0x050505, 100, 1000 ); // Dark fog to match dark theme
    ...
    const animate = () => {
        ...
        // Use a multiplier to make the birds scatter significantly from the cursor
        velocityUniforms[ 'predator' ].value.set( 0.5 * mouseX / windowHalfX, - 0.5 * mouseY / windowHalfY, 0 );

        // Slowly drift predator out of bounds if mouse is not moving
        mouseX += (10000 - mouseX) * 0.005; 
        mouseY += (10000 - mouseY) * 0.005;

        gpuCompute.compute();
        birdUniforms[ 'texturePosition' ].value = gpuCompute.getCurrentRenderTarget( positionVariable ).texture;
        birdUniforms[ 'textureVelocity' ].value = gpuCompute.getCurrentRenderTarget( velocityVariable ).texture;

        // Add subtle rotation to the whole scene for extra dynamism
...
window.initGPGPUBirds = initGPGPUBirds;
